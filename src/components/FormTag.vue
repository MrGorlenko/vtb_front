  ...
  <div class="form">
    <div class="form__progress">
      <div class="form__progress-names">
        <div class="form__progress-name form__progress-name--1">Выбор тарифа</div>
      ...
      <div class="form__progress-bar">
        <div class="form__progress-now"
          :class="{'form__progress-now--middle': state==2, 'form__progress-now--end': state==3}"
        ></div>
      </div>
    </div>
    <mq-responsive target="small+">
      <img class="form__figures-img" :src="require('@/assets/img/figures.webp')">
    </mq-responsive>
    <mq-responsive target="mobile">
      <img class="form__figures-img" :src="require('@/assets/img/figures-m.webp')">
    </mq-responsive>
    <mq-responsive target="small+">
      <img class="form__big-figures-img" :src="require('@/assets/img/big-figures.webp')">
    </mq-responsive>
    <mq-responsive target="mobile">
      <img class="form__big-figures-img" :src="require('@/assets/img/big-figures-m.webp')">
    </mq-responsive>
    <mq-responsive target="small+">
      <img class="form__bubble-img" :src="require('@/assets/img/bubble.webp')">
    </mq-responsive>
    <div class="form__tariffs" id="tariffs">
      <div class="form__tariff"
        :class="{'form__tariff--checked': data.tariff && (data.tariff.number==1),
      ...
      >
        ...
        <div class="form__tariff-cost">
          <span class="form__price">1 000 ₽</span><span class="form__cost-text">Стоимость</span>
        </div>
        <div class="form__tariff-button button" @click="selectTariff(0)">Выбрать</div>
      </div>
      <div class="form__tariff"
        :class="{'form__tariff--checked': data.tariff && (data.tariff.number==2),
      ...
      >
        ...
        <div class="form__tariff-cost">
          <span class="form__price">2 650 ₽</span><span class="form__cost-text">Стоимость</span>
        </div>
        <div class="form__tariff-button button" @click="selectTariff(1)">Выбрать</div>
      </div>
      <div class="form__tariff"
        :class="{'form__tariff--checked': data.tariff && (data.tariff.number==3),
      ...
      >
        ...
        <div class="form__tariff-cost">
          <span class="form__price">4 000 ₽</span><span class="form__cost-text">Стоимость</span>
        </div>
        <div class="form__tariff-button button" @click="selectTariff(2)">Выбрать</div>
      </div>
      <div class="form__tariff"
        :class="{'form__tariff--checked': data.tariff && (data.tariff.number==4),
      ...
      >
        ...
        <div class="form__tariff-cost">
          <span class="form__price">7 500 ₽</span><span class="form__cost-text">Стоимость</span>
        </div>
        <div class="form__tariff-button button" @click="selectTariff(3)">Выбрать</div>
        </div>
      </div>
    </div>
    <mq-responsive target="mobile" class="form__swipe">
      <div class="form__swipe-text">swipe</div>
      <div class="form__swipe-icon"><img class="form__swipe-arrow" :src="require('@/assets/img/arrow.svg')"/></div>
    </mq-responsive>
    <div class="form__form" id="form">
      <div class="form__caption">Укажите данные о себе</div>
      <div class="form__row">
        <custom-input type="text" class="form__input" v-model:value="data.full_name"
          placeholder="ФИО" :error="rules.full_name.error"
          :errorText="rules.full_name.errorText" @blur="validateProp('full_name')"/>
        <mq-responsive target="small+" class="form__input form__input--box input"
          :class="{'checked': data.gender==1}" @click="genderHandle(1)">М
          <div v-if="rules.gender.error" class="cu-input__error">
            <span class="cu-input__error-icon">*</span>
          </div>
        </mq-responsive>
        <mq-responsive target="small+" class="form__input form__input--box input"
          :class="{'checked': data.gender==2}" @click="genderHandle(2)">Ж
          <div v-if="rules.gender.error" class="cu-input__error">
            <span class="cu-input__error-icon">*</span>
          </div>
        </mq-responsive>
        <custom-input type="date" class="form__input form__input--mobile" :max="maxDate" :min="minDate"
          v-model:value="data.birthday" placeholder="Дата рождения" :error="rules.birthday.error"
          :errorText="rules.birthday.errorText" @blur="validateProp('birthday')"/>
        <custom-input type="text" class="form__input form__input--pc"
          v-model:value="data.birthday" placeholder="Дата рождения" :error="rules.birthday.error"
          :errorText="rules.birthday.errorText" @blur="validateProp('birthday')" mask="99.99.9999"/>
      </div>
      <div class="form__row">
        <custom-input type="tel" class="form__input form__input--bigger" v-model:value="data.passport"
          placeholder="Серия и номер паспорта" :error="rules.passport.error" mask="9999 999999"
          :errorText="rules.passport.errorText" @blur="validateProp('passport')"/>
        <custom-input type="date" class="form__input form__input--mobile" :min="minDate"
          v-model:value="data.p_date" placeholder="Дата выдачи паспорта"
          :error="rules.p_date.error"
          :errorText="rules.p_date.errorText" @blur="validateProp('p_date')"/>
        <custom-input type="text" class="form__input form__input--bigger form__input--pc"
          v-model:value="data.p_date" placeholder="Дата выдачи паспорта"
          :error="rules.p_date.error" mask="99.99.9999"
          :errorText="rules.p_date.errorText" @blur="validateProp('p_date')"/>
      </div>
      <mq-responsive target="mobile" class="form__row form__row--mobile">
        <div class="form__input form__input--box input"
          :class="{'checked': data.gender==1}" @click="genderHandle(1)"
          >М
          <div v-if="rules.gender.error" class="cu-input__error">
            <span class="cu-input__error-icon">*</span>
          </div>
        ...
        <div class="form__input form__input--box input"
          :class="{'checked': data.gender==2}" @click="genderHandle(2)"
        >Ж
          <div v-if="rules.gender.error" class="cu-input__error">
            <span class="cu-input__error-icon">*</span>
          </div>
      ...
      <div class="form__row form__row--bottom">
        <custom-input type="email" class="form__input" v-model:value="data.email"
          placeholder="Эл. почта" :error="rules.email.error"
          :errorText="rules.email.errorText" @blur="validateProp('email')"/>
        <custom-input type="tel" class="form__input" v-model:value="data.phone"
          placeholder="Моб. телефон" :error="rules.phone.error" mask="+7 (999) 999-99-99"
      ...
      <div class="form__note form__note--top" :class="{'visible': data.tariff}">
        ...
        ответственности по тарифу {{number}} стоимостью {{price}}
        руб. со сроком 12 месяцев</div>
    </div>
    <div class="form__button button" @click="confirmForm">
      <transition name="fade">
        <div v-if="rules.tariff.error || preloader.error" class="cu-input__error">
          <span class="cu-input__error-icon">*</span><span>{{errorText}}</span>
        </div>
      </transition>
      <span :class="{'hidden' : preloader.visible}" class="form__button-text">Оформить</span>
      <cu-preloader :visible="preloader.visible" class="form__preloader"/>
    </div>
    <div class="form__note">Нажимая кнопку “Оформить”, вы соглашаетесь на обработку ваших
      персональных данных и принимаете условия страхования</div>
  </div>
</template>
...
export default {
  ...
  data(){
    return{
      ...
      rules: {
        ...
        birthday: {required: true, error: false, complete: true, errorText: ''},
        ...
        passport: {required: true, error: false, complete: true, errorText: ''},
  ...
  computed:{
    ...
    maxDate(){
      let date = new Date();
      return new Date(date.setFullYear(date.getFullYear()-18)).toISOString().split('T')[0];
    },
    minDate(){
      return '1900-01-01';
    }
  ...
  methods:{
    validateProp(prop){
      ...
      if (prop=='p_date' || prop=='birthday'){
        if (!Date.parse(this.data[prop].split('.').reverse().join('-')) ||
            Date.parse(this.data[prop].split('.').reverse().join('-')) > Date.now()) {
          this.rules[prop].error = true;
    ...
    async confirmForm(){
      ...
      if (this.validateForm()) {
        ...
        this.axios.post('https://api-creatum.bexram.online/vtb/buy/', formData)
        .then( response => {
...
.form {
  ...
  &__progress-bar {
    background: linear-gradient(90deg, rgba(255, 255, 255, 0.4) 0%, rgba(255, 255, 255, 0.276) 100%);
  ...
  &__progress-now {
    ...
    background: linear-gradient(90deg, rgba(162, 89, 255, 0.8) 0%, rgba(9, 249, 191, 0.552) 100%);
    backdrop-filter: blur(10px);
    border-radius: 10px;
    height: 30px;
  ...
  &__tariff {
    ...
    &--checked{
      box-shadow: -13px -7px 38px rgba(225, 251, 255, 0.8), 10px 9px 90px rgba(97, 74, 211, 0.8);
      @media (max-width: 1440px){
  ...
  &__swipe-text {
    background: linear-gradient(80.19deg, rgba(162, 89, 255, 0.648) -60.35%, rgba(9, 249, 191, 0.15) 80.45%);
